...
class Player(pygame.sprite.Sprite):
    ...
    #untuk mengupdate
    def update(self):
        #jika pada fuel lebih dari 0 dan tidak dead dan game dimulai maka
        if self.fuel > 0 and not self.is_dead and self.has_started:
            #jika shield kurang dari sama dengan 0 maka akan dijalankan normal
            if self.shield <= 0:
                self.status = "NORMAL"
            #jika if tidak dijalankan maka dijalankan else yaitu status nya shielded
            else:
                self.status = "SHIELDED"

            #mendeklarasikan idle nya
            self.image = self.images[self.status]["IDLE"]
            self.state = "IDLE"
            #jika game dimulai maka dijalankan pressed
            if self.has_started:
                pressed = pygame.key.get_pressed()

                #jika pressed Key A atau pressed Key left 
                if pressed[pygame.K_a] or pressed[pygame.K_LEFT]:
                    #maka rect pada x sama dengan self pada rect dikurangi dengan speed pada x dikali 1.5
                    self.rect.x -= self.speedx * 1.5
                    self.image = self.images[self.status]["MOVLEFT"]
                    self.state = "MOVLEFT"
                #jika pressed Key D atau pressed K Right
                if pressed[pygame.K_d] or pressed[pygame.K_RIGHT]:
                    #maka rect pada x sama dengan self pada rect x ditambah dengan speed pada x nya
                    self.rect.x += self.speedx
                    self.image = self.images[self.status]["MOVRIGHT"]
                    self.state = "MOVRIGHT"
                #jika pressed Key W atau Pressed space atau pressed Key Up
                if pressed[pygame.K_w] or pressed[pygame.K_SPACE] or pressed[pygame.K_UP]:
                    #maka speed y sama dengan speed y dikurangi dengan gravity ditambah boost nya
                    self.speedy -= (self.GRAVITY + self.boost)
                #jika pressed Key S atau pressed K + L+ SHIFT atau pressed Key Down
                if pressed[pygame.K_s] or pressed[pygame.K_LSHIFT] or pressed[pygame.K_DOWN]:
                    #maka speed pada y sama dengan speed pada y ditambah  dengan kurung gravity ditambag boost dikali 0.2
                    self.speedy += (self.GRAVITY + self.boost * 0.2)

            #speed y sama dengan speed y ditambah gravity
            self.speedy += self.GRAVITY
            #rect pada y sama dengan rect y ditambah dengan speed y
            self.rect.y += self.speedy

        #elif game tidak berjalan maka (saat permulaan belum berjalan gamenya)
            ...
            self.rect.x += 1
            self.rect.y = 120
        #menjalankan else dengan mengubah image ke alpha 100, speed y sama dengan speed y ditambah gravity, rect y sama dengan rect y ditambah dengan speed y
        else:
            self.image.set_alpha(100)
            self.speedy += self.GRAVITY
            self.rect.y += self.speedy

    #mendefinisikan def draw
    def draw(self, window):
        window.blit(self.image, (self.rect.x,self.rect.y))

#cllass obstacle/rintangan
class Obstacle(pygame.sprite.Sprite):
    def __init__(self, obstacle_imgs, play_area):
        ...
        # For animation, jika gambar pada rintangannya lebih dari 1 maka mengeset timer dengan get_ticks, dengan di delay 100 dan tidak berframe
    ...
    def update(self):
        #jika obstacle_imgs lebih dari 1 maka dijalankan self.animate()
        if len(self.obstacle_imgs) > 1:
            self.animate()
        self.y += 0.1
        self.bob = math.sin(self.y) * self.multiplier
    ...
    #menjalankan def animate nya 
    def animate(self):
        #jika get_ticks() dikurangi anim_timer lebih dari anim_delay 
